          ...
          <a
            ...
            <div className="relative">
              <video
                ...
                loop
                muted
                className="w-36 h-16 transition-all duration-100 group-hover:scale-110 group-hover:rotate-6 group-hover:drop-shadow-[0_0_30px_rgba(100,255,255,0.8)]">
                <source src={logo} />
              </video>
            </div>
          ...
          <nav className="md:flex hidden items-center space-x-2 text-lg font-bold">
            {["home", "about", "projects", "contact"].map((item) => (
              ...
              rounded-lg transition-all duration-500 ${
                ...
                <div
                  className={`absolute -inset-1 bg-gradient-to-r from-blue-800 to-purple-800 rounded-lg blur-md opacity-0 
              transition-opacity duration-300 ${
                hoveredItem === item ? "opacity-70" : ""
              }`}></div>

                <div className="relative z-10 flex items-center">
                  <span
                    className={`block w-2 h-2 rounded-full mr-2 transition-all duration-300 ${
                      hoveredItem === item
                        ? "bg-cyan-400 shadow-[0_0_10px_2px_rgba(100,255,255,0.7)] scale-150"
                        : "bg-white/30"
                    }`}></span>
                  <span
                    className={`text-white/80 transition-all duration-300 ${
                      hoveredItem === item
                        ? "text-white tracking-wider scale-105"
                        : ""
                    }`}>
                ...
                <div
                  className={`absolute bottom-0 left-1/2 h-0.5 bg-gradient-to-r from-cyan-400 to-purple-600 
            transition-all duration-500 ${
              hoveredItem === item ? "w-4/5 -translate-x-1/2" : "w-0"
            }`}></div>
              </a>
            ))}
            `
