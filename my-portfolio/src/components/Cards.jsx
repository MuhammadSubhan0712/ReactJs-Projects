...
const Cards = ({ item }) => {
  ...
  return (
    <>
      <div
        className={`relative overflow-hidden rounded-xl border border-cyan-400/20 bg-gray-900/80 backdrop-blur-sm transition-all duration-500 w-full h-full ${
          isHovered ? "shadow-lg shadow-cyan-400/20" : ""
        }`}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}>
        {/* Hovered Effects: */}
        <div
          className={`absolute inset-0 bg-gradient-to-br from-cyan-400/10 to-purple-600/10 opacity-0 transition-opacity duration-300 ${
        ...
        <div className="relative overflow-hidden rounded-t-xl">
          <img
            ...
            className={`w-full h-48 object-cover transition-all duration-700 ${
              isHovered ? "scale-110 brightness-110" : "brightness-90"
            }`}
          />
        ...
        {/* Hover effects: */}
        <div
          className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-400 to-purple-600 transition-all duration-500 ${
